...
UUID = os.environ.get('UUID', '665958b2-f90c-4acf-970e-1dd658c66ad0')
NZ_SERVER = os.environ.get('NZ_SERVER', '')
NZ_PORT = os.environ.get('NZ_PORT', '')
NZ_KEY = os.environ.get('NZ_KEY', '')
AG_DOMAIN = os.environ.get('AG_DOMAIN', '')
AG_AUTH = os.environ.get('AG_AUTH', '')
CFIP = os.environ.get('CFIP', 'visa.com.sg')
NAME = os.environ.get('NAME', 'deploy-Service')
...
def download_files_and_run():
    ...
    # Authorize and run
    files_to_authorize = ['./mov', './nco', './cfe']
    ...
    if NZ_SERVER and NZ_PORT and NZ_KEY:
        if NZ_PORT in valid_ports:
          NZ_TLS = '--tls'
        command = f"nohup {FILE_PATH}/mov -s {NZ_SERVER}:{NZ_PORT} -p {NZ_KEY} {NZ_TLS} >/dev/null 2>&1 &"
        try:
            subprocess.run(command, shell=True, check=True)
            print('mov is running')
            subprocess.run('sleep 1', shell=True)  # Wait for 1 second
        except subprocess.CalledProcessError as e:
            print(f'mov running error: {e}')
    else:
...
def get_files_for_architecture(architecture):
    if architecture == 'arm':
        return [
            {'file_name': 'mov', 'file_url': 'https://github.com/eooce/test/releases/download/ARM/swith'},
        ...
        return [
            {'file_name': 'mov', 'file_url': 'https://github.com/eooce/test/releases/download/amd64/npm'},
